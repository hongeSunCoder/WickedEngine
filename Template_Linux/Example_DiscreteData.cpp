...
#include "Discrete_Data/discretedata.h"
#include "Discrete_Data/discretedata_internal.h"
...
{
    return DiscreteData::GetCurrentContext() ? (ImGui_Impl_Data *)DiscreteData::GetIO().BackendRendererUserData : nullptr;
}
...
#endif
    DiscreteData::DestroyContext();
}

void Example_DiscreteData::Initialize()
{

    DiscreteData::CreateContext();
    ImGuiIO &io = DiscreteData::GetIO();
    (void)io;
    ...
    printf("-----end Example_DiscreteData::Initialize ------\n");
}

void Example_DiscreteData::Compose(wi::graphics::CommandList cmd)
{
    printf("------ start Example_DiscreteData::Compose ----------\n");
    Application::Compose(cmd);

    DiscreteData::Render();

    auto drawData = DiscreteData::GetDrawData();
    ...
    {
        const DiscreteDrawList *drawList = drawData->CmdLists[cmdListIdx];
    ...
    for (uint32_t cmdListIdx = 0; cmdListIdx < (uint32_t)drawData->CmdListsCount; ++cmdListIdx)
    {
        ...
        {
            const DiscreteDrawCmd *drawCmd = &drawList->CmdBuffer[cmdIndex];
    ...
    printf("-----end Example_DiscreteData::Compose -------\n");
}
...
{
    printf("----start Example_DiscreteDataRenderer::Load --------\n");
    setSSREnabled(false);
    ...
    RenderPath3D::Load();
    printf("----end Example_DiscreteDataRenderer::Load --------\n");
}
...
{
    printf("----start Example_DiscreteDataRenderer::Update --------\n");
...
#endif
    DiscreteData::NewFrame();

    // 1. Show the big demo window (Most of the sample code is in ImGui::ShowDemoWindow()! You can browse its code to learn more about Dear ImGui!).
    // if (show_demo_window)
    //     DiscreteData::ShowDemoWindow(&show_demo_window);
    ...
    // {
    // DiscreteData::Begin("Another Window", &show_another_window); // Pass a pointer to our bool variable (the window will have a closing button that will clear the bool when clicked)
    // DiscreteData::Text("Hello from another window!");
    // if (DiscreteData::Button("Close Me"))
    ...
    printf("----end Example_DiscreteDataRenderer::Update --------\n");
}
